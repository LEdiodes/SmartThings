 ...
 *
 *  Copyright (c) 2014 Matthew Kerfoot(https://github.com/mkerfoot)
 ...
 *  Install Steps
 *  ----------------
 *  1. Create new SmartApps at https://graph.api.smartthings.com/ide/apps using the SmartApp at https://github.com/mkerfoot/SmartThings.CycleLEDStrips.
...
def mainPage() {
	dynamicPage(name: "mainPage") {
		section("Select bulbs\LED strips") {
			input "hues", "capability.colorControl", title: "Select Hue Bulbs\LEDs strips", required:true, multiple:true
		}
		section("Options...") {
			...
			input "cycleSec", "integer", title: "Select color change interval", required: true, defaultValue:5
			input "cycleStep", "integer", title: "Select how many steps between switching colors", required: true, defaultValue:1
		}
...
def installed() {
	log.debug "Installed with the following settings: ${settings}"
	subscribeToEvents()
}

def updated() {
	log.debug "Updated with the following settings: ${settings}"
	unsubscribe()
	unschedule()
	subscribeToEvents()
}
